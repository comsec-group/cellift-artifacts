...
def worker(ift_flavor_data_width_pair):
    ...
    print(verilator_run_output_file_path)
    with open(verilator_run_output_file_path, "w") as out_file:
        proc = subprocess.run("./Vmycell".format(ift_flavor, data_width, NUM_EXECUTIONS, cell_name).split(" "), env=os.environ, stdout=out_file, stderr=out_file, shell=True, cwd="build/{}/build/mycell_0.1/default-verilator".format(cell_name))
