# This script is used to create a single plot for the CellIFT slides.
# It only plots performance, and not precision.
...
fig, ax = plt.subplots(1, num_cells)
ax.set_ylabel('Simulation runtime for 1M random inputs (ms)')
fig.tight_layout()
...
for cell_id in range(num_cells):
    ...
    performance_axes.append(ax)
    # if cell_id == num_cells-1:
    #     newx.set_ylabel('Taint (million bits)')
...
for cell_id, cell in enumerate(cells):
    ...
    for instr_method in cell_data[cell]:
        if instr_method == "rtlift":
            continue
        performance_axes[cell_id].plot(cell_data[cell][instr_method]["performance"]['x'], cell_data[cell][instr_method]["performance"]['y'], line_marker["performance"][instr_method], color=line_color["performance"][instr_method], linewidth=line_width["performance"][instr_method], label="Performance {}".format(instr_method_names[instr_method]))
...
performance_handles, performance_labels = ax.get_legend_handles_labels()

# fig.legend(performance_handles, performance_labels, bbox_to_anchor=(0, 0, 1, 1), loc='upper center', ncol=len(performance_handles))
ax.legend(performance_handles, performance_labels)

# Adjust the plot.
# fig.subplots_adjust(wspace=1.0, hspace=0.1, top=0.8)
ax.set_xlabel("Cell input width (bit)")

ax.set_title(cell_names[cells[0]])
fig.tight_layout()

# Write the plot.
pathlib.Path("fig").mkdir(parents=True, exist_ok=True)

plt.savefig('fig/ubench-performance_single_nortlift.png', dpi=300)
plt.savefig('fig/ubench-performance_single_nortlift.pdf', dpi=300)
