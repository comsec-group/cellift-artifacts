...
long long int count_tainted_outputs(Testbench *tb, uint32_t& execution_time_micros) {
  ...
  for (int exec_id = 0; exec_id < kNumExecutions; exec_id++) {
    for (int instance_id = 0; instance_id < kNumInstances; instance_id++) {
      for (int mask_elem_id = 0; mask_elem_id < kNumIntsIn; mask_elem_id++) {
        tb->module_->a_i[4*instance_id+mask_elem_id]    = ((rand() & 0xFFFF) << 16) | (rand() & 0xFFFF);
        tb->module_->a_i_t0[4*instance_id+mask_elem_id] = ((rand() & 0xFFFF) << 16) | (rand() & 0xFFFF);
      }
      for (int mask_elem_id = kNumIntsIn; mask_elem_id < 4; mask_elem_id++) {
        tb->module_->a_i[4*instance_id+mask_elem_id]    = 0;
        tb->module_->a_i_t0[4*instance_id+mask_elem_id] = 0;
      }
    }
