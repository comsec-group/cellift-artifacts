...
const int OutDataWidth = DATA_WIDTH+1;
const int kNumIntsIn  = (DATA_WIDTH+31)/32; // Num integers.
const int kNumIntsOut = (OutDataWidth+31)/32; // Num integers. +32 because the output is 1 bit larger.
int kCellInWidth = 2;
...
long long int count_tainted_outputs(Testbench *tb, uint32_t& execution_time_micros) {
  ...
  for (int mask_elem_id = 0; mask_elem_id < kNumIntsOut; mask_elem_id++) {
    int num_bits_set = std::min(OutDataWidth-32*mask_elem_id, 32);
